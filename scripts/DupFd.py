...
sql1 = "select distinct split_part(message, ': ', 2),device_id from %(project)s_nm_log where %(where)s order by message;"
            ...
            for fdrow in fd_rows:
                ...
                if match:
                    ...
                    if not fd:
                        ...
                        dups[fdnew] = "Different FD: %s" % ",".join([str(x) for x in fdrow])
